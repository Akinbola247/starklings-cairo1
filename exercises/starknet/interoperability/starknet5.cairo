    ...
    impl ContractAImpl of super::IContractA<ContractState> {
        fn set_value(ref self: ContractState, value: u128) -> bool {
            ...
            // If it is, set the value and return true. Otherwise, return false.
             let contract__B = self.contract_b.read();
             let dispatcher = IContractBDispatcher {
                contract_address: contract__B
            };
            dispatcher.enable();
            let contract_b_status = dispatcher.is_enabled();
            if contract_b_status == true {
